// ===================================================
//  AtomS3 Secondary - v0.19.3-I2CFIX
//  - FIXED: Protocol Mismatch with Master v0.19.2
//  - ADDED: CRC8 Checksum & Packet Headers
//  - ADDED: HDOP Telemetry
//  - FIXED: Missing zeroIMU prototype & implementation
// ===================================================
#include <M5Unified.h>
#include <Wire.h>
#include <TinyGPS++.h>
...
// ========== CONFIGURATION ==========
#define FW_VERSION "v0.19.3-PROTO-FIX"
...
struct CompactTelemetry {
  ...
  uint32_t time;
  int16_t temp_c_x10;
  uint16_t hdop_x10;  // ✅ ADDED: Missing field
} __attribute__((packed)) i2c_data;

// 2. The packet wrapper (matches Master expectation)
struct TelemetryPacket {
  uint8_t header;
  uint8_t version;
  uint8_t length;
  CompactTelemetry payload;
  uint8_t crc;
...
void handleButton();
void calibrateIMU();
void zeroIMU(); // ✅ ADDED: Missing prototype
void saveTopSpeeds();
void loadTopSpeeds();
...
// ✅ FIXED: Enhanced Zero function with filter reset
void zeroIMU() { 
  // 1. Run hardware calibration
  calibrateIMU(); 
  
  // 2. Reset filters to prevent visual drift
  yaw_rate_filtered = 0.0f;
  yaw_rate_raw = 0.0f;
  i2c_data.gforce_lat = 0.0f;
  i2c_data.gforce_lon = 0.0f;
  i2c_data.gforce_total = 0.0f;
  
  // 3. Feedback
  showActionFeedback("ZEROED", COLOR_INFO);
  Serial.println("[IMU] Zeroed & Filters Reset");
}
...
// ✅✅✅ FIXED: PROTOCOL WRAPPER FOR MASTER V0.19.2 ✅✅✅
void requestEvent() {
  noInterrupts();
  i2c_request_count++;
  last_i2c_activity = millis();
  ...
  packet.length = sizeof(CompactTelemetry);
  packet.payload = i2c_data;
  
  // 3. Calculate CRC
  packet.crc = crc8((uint8_t*)&packet.payload, sizeof(CompactTelemetry));
  
  interrupts();
  
  // 4. Send Full Packet
  Wire.write((uint8_t*)&packet, sizeof(TelemetryPacket));
}
...
void updateSensorData() {
  if (gps.location.isValid() && gps.location.age() < 2000) {
    ...
    i2c_data.hdop_x10 = (uint16_t)(gps.hdop.value() / 10); // ✅ Capture HDOP
